...
: Drawer("Bzr", width, height, palSize), m_colorIndex(0), m_camera(camera) {
    ...
    m_settingsRanges.insert(std::make_pair("params", std::make_pair(0,7)));
        ...
        for (int y = 0; y < m_bzrHeight; ++y) {
            for (int x = 0; x < m_bzrWidth; ++x) {
                ...
                for (int j=y-1; j<=y+1; j++) {
                    int jj = (j + m_bzrHeight) % m_bzrHeight;
                    for (int i=x-1; i<=x+1; i++) {
                        size_t ii = (i + m_bzrWidth) % m_bzrWidth;
                        size_t ind = ii + jj * m_bzrWidth;
                        c_a += m_a[m_q]->get(ind);
                        c_b += m_b[m_q]->get(ind);
                        c_c += m_c[m_q]->get(ind);
                    }
