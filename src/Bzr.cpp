    ...
    if (m_state == 0) {
        for (int y = 0; y < m_height; ++y) {
            for (int x = 0; x < m_width; ++x) {
                ...
                m_a[1 - m_q]->get(ind) = std::min(std::max(c_a + c_a * ( c_b - c_c ), 0.0f), 1.0f);
                m_b[1 - m_q]->get(ind) = std::min(std::max(c_b + c_b * ( c_c - c_a ), 0.0f), 1.0f);
                m_c[1 - m_q]->get(ind) = std::min(std::max(c_c + c_c * ( c_a - c_b ), 0.0f), 1.0f);
            }
        }
        m_q = 1 - m_q;
    }
